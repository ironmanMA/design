package com.ironman.ma.Array.merge.intervals;
...
public class Solution {
    ...
    private boolean isStartBetween(Interval existInterval, Interval insertInterval) {
        if (insertInterval.start >= existInterval.start && insertInterval.start <= existInterval.end)
            return true;
        return false;
    }

    private boolean isEndBetween(Interval existInterval, Interval insertInterval) {
        if (insertInterval.end <= existInterval.end)
            return true;
        return false;
    }
